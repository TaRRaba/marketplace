...
export const GoodsList = ({ CatId }: { CatId: number }) => {
  ...
  function initFilters() {
    if (goods.length > 0) {
      const uniqueCountryValues = [
        ...new Set(goods.map((good) => good.country)),
      ];
      const uniqueBrandValues = [
        ...new Set(goods.map((good) => good.specs.brand)),
      ];
  ...
  const filterHandler = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    function filterProducts(
      products: IGoodData[],
      countries: FormDataEntryValue[] | null,
      brands: FormDataEntryValue[] | null,
      ...
      const filteredProducts = products.filter((product) => {
        const isCountryMatch = countries
          ...
          : true;
        const isBrandMatch = brands
          ? brands.includes(product.specs.brand)
          : true;
    ...
    if (goods.length > 0) {
      ...
      setGoods(
        filterProducts(initGoods, filterCountry, filterBrand, filterPrice)
      );
    }
  ...
  function SubCatFilter(id: number) {
    const filteredProducts = [
      ...initGoods.filter((good) => good.subcategory_id === id),
    ];
  ...
  function restoreGoods() {
    ...
    const countries: NodeListOf<HTMLElement> =
    ...
    (async function () {
      try {
        const response = await fetch(
          `http://localhost:3001/api/fav/category/${CatId}`,
          {
            credentials: "include",
          }
        );
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              >
                ...
                >
                  {subCat.map((category) => (
                    <li key={category.name}>
                      <div
                        onClick={() => SubCatFilter(category.id)}
                        className="text-lg hover:bg-teal-500/30 cursor-pointer rounded-md py-2 px-4 font-medium"
                      >
                        {category.name}
                      </div>
                    </li>
                ...
                <div className="flex mt-5 justify-center">
                  <button
                    type="submit"
                    className="flex mr-3 text-sm justify-center w-28 cursor-pointer space-x-1.5 rounded-lg hover:bg-teal-500 bg-teal-600 px-4 py-1.5 text-white duration-100"
                  >
                    Применить
                  </button>
                  <div
                    onClick={restoreGoods}
                    className="flex text-sm justify-center w-28 cursor-pointer space-x-1.5 rounded-lg hover:bg-teal-500 bg-teal-600 px-4 py-1.5 text-white duration-100"
                  >
                    Сбросить
                  </div>
                </div>
              ...
              <div
                ...
                className="overflow-y-scroll lg:col-span-3"
              >
                <>
                  {goods.length > 0 ? (
                    <div
                      id="Cart"
                      className="visibility: visible bg-stone-100 py-10 min-h-96"
                    >
                                ...
                                archive === false && (
                                  ...
                                  >
                                    <div className="rounded-lg w-40 h-40 ">
                                      <img
                                        src={`http://localhost:3001${img_url}`}
                                        alt=""
                                    ...
                                    <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                                      <div className="mt-5 w-80 sm:mt-0">
                                        <Link to={`/goods/${name}`}>
                                          <h2 className="line-clamp-3 text-left text-lg font-bold text-gray-900 hover:text-gray-500">
                                            {name}
                                          </h2>
                                        </Link>
                                        {amount === 0 ? (
                                          <p className="text-left mt-3 text-md text-red-600">
                                            Нет в наличии
                                          </p>
                                        ) : amount > 10 ? (
                                          <p className="text-left mt-3 text-md text-green-700">
                                            В наличии
                                          </p>
                                        ) : (
                                          <p className="text-left mt-3 text-md text-yellow-500">
                                            Осталось мало
                                          </p>
                                        )}
                                        <div className="flex mt-5 w-80 sm:mt-0">
                                          <p className="text-left mt-5 text-xs text-gray-500">
                                            Страна: {country}
                                          </p>
                                          <p className="ml-5 text-left mt-5 text-xs text-gray-500">
                                            Бренд: {specs.brand}
                                          </p>
                                        </div>
                                      </div>
                                          ...
                                          <>
                                            {checkFav(id) ? (
                                              <svg
                                                ...
                                                className="addFav h-5 w-5 self-end cursor-pointer duration-150 text-red-500"
                                              ...
                                              <svg
                                                ...
                                                className="addFav h-5 w-5 self-end cursor-pointer duration-150 hover:text-red-500"
                                                xmlns="http://www.w3.org/2000/svg"
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                strokeWidth="1.5"
                                              ...
                                              <>
                                                {checkCart(id) ? (
                                                  ...
                                                  <div
                                                    ...
                                                    className="addToCart mt-4 flex text-sm items-center cursor-pointer space-x-1.5 rounded-lg bg-[#0d7490] px-4 py-1.5 text-white duration-100 hover:bg-[#0d7490]/80"
                                                  >
                                                    <svg
                                                      xmlns="http://www.w3.org/2000/svg"
                                                      fill="none"
                                                      ...
                                                      strokeWidth="1.5"
                                                      stroke="currentColor"
                                                      className="addToCart mr-2 h-4 w-4"
                                                    >
                                                      <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                    ...
                    <section className="py-10 flex bg-gray-100">
                      <div className="text-2xl my-40 justify-center text-gray-700 m-auto">
