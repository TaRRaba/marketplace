              ...
              {favourites && favourites.map(({ id, Good }) => (
                ...
                <p className="ml-5 text-left mt-5 text-xs text-gray-500">Производитель: {Good.specs.brand}</p>
                    ...
                    <div className="flex flex-col w-28 mt-0">
                      ...
                      {checkCart(Good.id) ?
              <div className="addedCart flex text-sm items-center cursor-pointer justify-center space-x-1.5 rounded-lg bg-teal-600 px-4 py-1.5 text-white duration-100">
              В корзине
            </div>
              :
             <div onClick={() => dispatch(addAmountCart({goodID: Good.id, amount: 1}))} className="addToCart flex text-sm items-center cursor-pointer space-x-1.5 rounded-lg bg-[#4520aa] px-2 py-1.5 text-white duration-100 hover:bg-[#4520aa]/80">
