...
export const Cart = () => {
  ...
  const pickpointAddress = useAppSelector(
    (state: RootState) => state.cart.pickpointAddress
  );
  ...
  return (
    <>
      {cart.length > 0 ? (
        <div
          id="Cart"
          className="visibility: visible bg-neutral-50 mb-44 min-h-96 pt-10"
        >
          {billData ? (
              ...
              <div id="amount" className="rounded-lg md:w-2/3">
                {cart &&
                  cart.map(({ id, quantity, Good }) => (
                    ...
                    >
                      <img
                        src={`http://localhost:3001${Good.img_url}`}
                        alt=""
                        className="rounded-lg w-40"
                      />
                      <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                        <div className="mt-5 w-80 sm:mt-0">
                          <a href={`/goods/${Good.name}`}>
                          ...
                          <div className="flex mt-5 w-80 sm:mt-0">
                            ...
                            <p className="ml-5 text-left mt-5 text-xs text-gray-500">
                              Бренд: {Good.specs.brand}
                            </p>
                          </div>
                        </div>
                        <div className="flex flex-col mt-0">
                          <svg
                            onClick={() => dispatch(delPos({ entryId: id }))}
                            className="delPos h-5 w-5 self-end cursor-pointer duration-150 hover:text-red-500"
                          ...
                          <div className="flex mt-16 mb-5 self-end items-center border-gray-100">
                            <svg
                              onClick={() =>
                                dispatch(decrAmount({ entryId: id, quantity }))
                              }
                              className="amountDecr cursor-pointer text-gray-500 focus:outline-none focus:text-gray-600 h-5 w-5"
                              fill="none"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              strokeWidth="2"
                            ...
                            >
                              <path
                                className="amountDecr"
                                d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"
                              />
                            </svg>
                            <span className="quantity text-gray-700 text-lg mx-2">
                              {quantity}
                            </span>
                            <svg
                              onClick={() =>
                              ...
                              className="amountIncr text-gray-500 cursor-pointer focus:outline-none focus:text-gray-600 h-5 w-5"
                              fill="none"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              strokeWidth="2"
            ...
            <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                  ...
                  {deliveryState ? (
                    <>
                      ...
                      <input
                        ...
                        className="w-full py-1.5 px-1 border-2 border-gray-500 rounded-lg text-start"
                      ></input>
                    </>
                  ) : (
                    <div className="relative w-full mt-2 rounded-lg">
                      ...
                      <select
                        name="pickpoint"
                        className="relative w-full mt-4 bg-gray-100 rounded-lg"
                        value={pickpointAddress}
                        onChange={(e) =>
                          dispatch(setPickpointAddress(e.target.value))
                        }
                      >
                        {address &&
                          address.map((el) => (
              ...
              {billData ? (
                ...
                <button
                  ...
                  onClick={handleBill}
                  type="button"
                  className="mt-6 w-full rounded-md py-1.5 font-medium text-blue-50 bg-[#0d7490] hover:bg-[#0d7490]/80"
                >
                  Перейти к оплате
                </button>
              )}
