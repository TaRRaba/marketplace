      ...
      {card && card.map(({ id, name, price, amount, img_url }) => (
        <article key={id} id={id} className="grid grid-cols-1 article content-between rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <> 
            {checkCart(id) ?
  <div className="addedCart flex text-sm items-center cursor-pointer space-x-1.5 rounded-lg bg-green-700 px-4 py-1.5 text-white duration-100">
  В корзине
</div>
  :
 <div onClick={() => dispatch(addAmountCart({goodID: id, amount: 1}))} className="addToCart flex text-sm items-center cursor-pointer space-x-1.5 rounded-lg bg-[#0d7490] px-4 py-1.5 text-white duration-100 hover:bg-[#0d7490]/80">
